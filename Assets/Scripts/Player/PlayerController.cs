...
namespace HyperCasual.Runner
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField]
        DynamicJoystick m_Joystick; // Reference to Dynamic Joystick
        ...
        public void ChangeColor(Color newColor)
        {
            if (m_SkinnedMeshRenderer != null)
            {
                // Assuming you have a SkinnedMeshRenderer component
                m_SkinnedMeshRenderer.material.color = newColor;
            }
            else
            {
                // If you have a different way of handling the player's color, adjust the code here
                Debug.LogWarning("SkinnedMeshRenderer is missing. Unable to change color.");
            }
        }
        ...
        void Update()
        {
            ...
            if (!m_AutoMoveForward)
            {
                ...
                {
                    // No joystick input, stop the player
                    Decelerate(deltaTime, 0.0f);
                }
                else
                {
                    // Joystick input, move the player
                    SetDeltaPosition(horizontalInput);
                }
            }
